...
import edu.seb.chaos.now.assault.filters.MethodsFilter;
...
public class Configuration {
    ...
    @Bean
    public ExceptionAssaultInvoker exceptionAssaultRequestScope() {
        return new ExceptionAssaultInvoker(settings());
    }
    ...
    @Bean
    public DefaultPointcutAdvisor targetablePointcutAdvisor(ClassFilter classFilter, ExceptionAssaultInvoker requestScope) {
        return new DefaultPointcutAdvisor(
                new ComposablePointcut(ClassFilters.intersection(classFilter, new AnnotationClassFilter(Targetable.class)),
                        MethodsFilter.INSTANCE), new Advice(requestScope));
    }
    ...
    public DefaultPointcutAdvisor killablePointcutAdvisor(ClassFilter classFilter) {
        return new DefaultPointcutAdvisor(
                new ComposablePointcut(ClassFilters.intersection(classFilter, new AnnotationClassFilter(Killable.class)),
                        MethodsFilter.INSTANCE), new KillableAdvice(killAssault()));
    }
